...
export const SubscriptionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [showWorkoutGenerationModal, setShowWorkoutGenerationModal] = useState(false);
  ...
  useEffect(() => {
    if (isOnAuthPage) {
      ...
      setShowWorkoutGenerationModal(false);
    }
  }, [isOnAuthPage]);
  ...
  const subscribe = async (type: SubscriptionType) => {
    ...
    if (success) {
      ...
      if (!hasCompletedSurvey) {
        ...
        // If survey is completed, check for auto-generated workouts
  ...
  const value = {
    ...
    showWorkoutGenerationModal: isOnAuthPage ? false : showWorkoutGenerationModal, // Force false on auth pages
    subscribe,
    startTrial,
    ...
    setShowWorkoutGenerationModal: (show: boolean) => {
      // Don't allow setting to true on auth pages
      if (!isOnAuthPage || !show) {
        setShowWorkoutGenerationModal(show);
      }
    },
    checkSubscription,
  };
